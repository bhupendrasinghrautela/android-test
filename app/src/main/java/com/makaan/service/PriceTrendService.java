...
public class PriceTrendService  implements MakaanService{
    ...
    public void getPriceTrendForLocalities(ArrayList<Long> topLocalityIds, int monthDuration, LocalityTrendCallback localityTrendCallback) {
        ...
        if (null != topLocalityIds && topLocalityIds.size() > 0 && monthDuration > 0) {
            ...
            priceTrendUrl.append("&").append(MONTH_DURATION).append("=").append(monthDuration).append("&").append(FILTERS).append("=");
    ...
    public void getCityPriceTrendForCity(Long cityId, int monthDuration){
        StringBuilder priceTrendUrl = new StringBuilder(ApiConstants.CITY_TREND_URL);
        priceTrendUrl.append("&").append(MONTH_DURATION).append("=").append(monthDuration).append("&").append(FILTERS).append("=cityId==").append(cityId.toString());
