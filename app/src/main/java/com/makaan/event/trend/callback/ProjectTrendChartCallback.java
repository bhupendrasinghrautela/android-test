                    ...
                    if (null != localityTrendData) {
                        for (Map.Entry<String, List<ApiPriceTrendData>> trendEntry : localityTrendData.entrySet()) {
                            ...
                            if (null != apiPriceTrendList && apiPriceTrendList.size() > 0) {
                                ...
                                Long minPricePerUnitArea = apiPriceTrendData.extraAttributes.get("minBuyPricePerUnitArea");
                                if (null != minPricePerUnitArea) {
                                    projectPriceTrendDto.addPriceTrendData(projectId, apiPriceTrendData.projectName, epochDataDate, minPricePerUnitArea);
                                }
                            }
